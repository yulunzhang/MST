...
def upsample_resize_images(inputs, scale, data_format='channels_first', method='ResizeMethod.NEAREST_NEIGHBOR'):
    shape = tf.shape(inputs)
    if data_format == 'channels_first':
        n, c, h, w = shape[0], shape[1], shape[2], shape[3]
        inputs = tf.transpose(inputs, [0, 2, 3, 1])
        inputs = tf.image.resize_images(inputs, tf.stack([h*scale, w*scale]), method)
        return tf.transpose(inputs, [0, 3, 1, 2])
    else:
        n, h, w, c = shape[0], shape[1], shape[2], shape[3]
        return tf.image.resize_images(inputs, tf.stack([h*scale, w*scale]), method)

def upsample_resize_bilinear(inputs, scale=2, data_format='channels_first'):
    shape = tf.shape(inputs)
    if data_format == 'channels_first':
        n, c, h, w = shape[0], shape[1], shape[2], shape[3]
        inputs = tf.transpose(inputs, [0, 2, 3, 1])
        inputs = tf.image.resize_bilinear(inputs, tf.stack([h*scale, w*scale]))
        return tf.transpose(inputs, [0, 3, 1, 2])
    else:
        n, h, w, c = shape[0], shape[1], shape[2], shape[3]
        return tf.image.resize_bilinear(inputs, tf.stack([h*scale, w*scale]))


def upsample_resize_bicubic(inputs, scale=2, data_format='channels_first'):
    shape = tf.shape(inputs)
    if data_format == 'channels_first':
        n, c, h, w = shape[0], shape[1], shape[2], shape[3]
        inputs = tf.transpose(inputs, [0, 2, 3, 1])
        inputs = tf.image.resize_bicubic(inputs, tf.stack([h*scale, w*scale]))
        return tf.transpose(inputs, [0, 3, 1, 2])
    else:
        n, h, w, c = shape[0], shape[1], shape[2], shape[3]
        return tf.image.resize_bicubic(inputs, tf.stack([h*scale, w*scale]))

def downsample_resize_bicubic(inputs, scale=2, data_format='channels_first'):
    shape = tf.shape(inputs)
    if data_format == 'channels_first':
        n, c, h, w = shape[0], shape[1], shape[2], shape[3]
        inputs = tf.transpose(inputs, [0, 2, 3, 1])
